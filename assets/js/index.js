...
searchInput.addEventListener("change", (event) => {
    fetch(`https://api.openweathermap.org/data/2.5/weather?q=${event.target.value}&appid=${API_KEY}&lang=vi&units=metric`)
        ...
        .then((data) => {
            ...
            sunrise.innerHTML = Math.round(data.sys.sunrise) || DEFAULT_VALUE;          
            sunset.innerHTML = Math.round(data.sys.sunset) || DEFAULT_VALUE;          
            windSpeed.innerHTML = Math.round(data.wind.speed) || DEFAULT_VALUE;          
            humidity.innerHTML = Math.round(data.main.humidity) || DEFAULT_VALUE;          
        })
})